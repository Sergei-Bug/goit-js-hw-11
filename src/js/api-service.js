...
export async function fetchInfo(searchThis) {
  page = 1;
  const responsInfo = await fetch(
    `${BASE_URL}?key=${KEY}&q=${searchThis}&image_type=photo&orientation=horizontal&safesearch=false&page=${page}&per_page=40&lang=ru`
  );
...
export async function getMore(searchThis) {
  page += 1;
  const responsMoreInfo = await fetch(
    `${BASE_URL}?key=${KEY}&q=${searchThis}&image_type=photo&orientation=horizontal&safesearch=false&page=${page}&per_page=40&lang=ru`
  );
